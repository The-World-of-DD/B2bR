...
echo "#vCPU : $(nproc)"
echo "#Memory Usage: $(free -m | awk '/Mem/ {printf "%d/%dMb (%.2f%%)", $3, $2, ($3/$2)*100}')"
...
echo "#User log: $(w -h | grep -v "tty" | wc -l)"
echo "#Network: IP $(hostname -I)($(ip link show | awk '/link\/ether/ {print $2}'))"
echo "#Sudo: $(grep "COMMAND=" /var/log/sudo/sudo.log | wc -l)"
} | wall
